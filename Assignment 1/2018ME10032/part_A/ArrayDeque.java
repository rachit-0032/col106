...
public class ArrayDeque implements DequeInterface {
  ...
  public void createDouble(){
    ...
    front = 0;
    rear = deque.length; //since these two varibles indicate the index and not the number!
  ...
  public void insertFirst(Object o){
    ...
    //Now that we have set the position of front where the element should be placed, check if rear also is there
    //Because if it is, it means that array is already full and you need to create a new array of double the size
    // if(front == rear){
    //   createDouble(); //create an array with double the size and copy all the elements in order
    //   front = deque.length - 1; //because front is meant to be 0 in the new array!
    // }
    deque[front] = o;
  ...
  public Object removeFirst() throws EmptyDequeException{
    ...
    else if(front+1 == deque.length){ //because if rear is at index 0, it means that we need to print out the element at the end of array and remove the circling effect
    //Also, rear points to the position where the next element could enter
      Object firstEle = deque[deque.length - 1];
      deque[front] = null;
      front = 0;
      return firstEle;
    }
  ...
  public int size(){
    //you need to implement this
    if(isEmpty() == false){ //and it is obviously not full because that has already been checked above
      if(front < rear)
        return rear - front;
      else{
  ...
  //All corner cases have been dealth with in my opinion
