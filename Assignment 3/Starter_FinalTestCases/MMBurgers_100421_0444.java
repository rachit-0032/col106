...
//THIS IS ONLY FOR Storing the time and queue number for first person in each queue so that while removing elements, we know which one to process first
...
public class MMBurgers implements MMBurgersInterface {
    ...
    public void advanceTime(int t) throws IllegalNumberException{
        //your implementation
        if(globalClock > t)
            throw new IllegalNumberException("This time has already passed and the simulation has been executed! Can't go back in time or rather, nothing to advance to!");
        else if (t<0)
        ...
        while(globalClock <= t){
            ...
            if (globalClock != t){
                int time1 = globalClock+1;
                int time2 = globalClock+1;
                int time3 = globalClock+1;

                if(currentQSizes.minElement().key1!=0){ //minimum sized-queue has some elements
                    time1 = firstPeopleToBeExecuted.minElement().key1; //time can come for the person at the billing counter
                }
                if(pattySequence.size() != 0){
                    time2 = pattySequence.getFirst();
                }
                if(deliveryQueue.size() != 0){
                    time3 = deliveryQueue.peek().waitTime;
                }

                globalClock = Math.min(Math.min(time1, time2), time3);
                // globalClock++; 
            }
    ...
    public void arriveCustomer(int id, int t, int numb) throws IllegalNumberException{
        ...
        if(numb<0) //########## Think about if you are implementing the ZERO Case correctly or not
            throw new IllegalNumberException("Number of Burgers entered has to be positive");
        ...
        if(t == globalClock){ //t == globalClock; this cannot come in else part since customer has to be added after advancing time
            ...
            if(billingCounters.get(addToQueue).peek() == null){ //if it is the first customer to be added 
                ...
                GriddleNode addingCustomer = new GriddleNode(id, numb, latestCustTime+addToQueue);
                billingCounters.get(addToQueue).add(addingCustomer); //added to the actual billing counters too
                currentQSizes.addCustomer(); //added into the heap which stores size of the queue
                //firstPeopleToBeExecuted is not utilised here since there is already someone there!
                System.out.println("Time of Execution for this: " + addingCustomer.waitTime);
                System.out.println();
                GlobalRepository.insert(id, numb, latestCustTime+addToQueue, addToQueue, t); //customer added to the AVL Tree
                latestCustTime = addingCustomer.waitTime; //updating this time with the exection time for the customer added
                totalCustomers++;
            }  
        }
